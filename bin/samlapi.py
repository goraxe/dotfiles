...
idpentryurl =             'https://dco-auth.sophos.net:443/adfs/ls/idpinitiatedsignon?loginToRp=urn:amazon:webservices'
#idpentryurl =             'https://dco-auth.sophos.net:443/adfs/ls/idpinitiatedsignon'
# 'https://dco-auth.sophos.net/adfs/ls/idpinitiatedsignon?loginToRp=urn:amazon:webservices' 
...
if len(awsroles) > 1: 
    ...
    if int(selectedroleindex) > (len(awsroles) - 1): 
        ...
        sys.exit(0) 
    print awsroles[int(selectedroleindex)]
...
print "role_arn: >>%s<< principal_arn: >>%s<< assertion: %s" % (role_arn, principal_arn, assertion)
# Use the assertion to get an AWS STS token using Assume Role with SAML
conn = boto.sts.connect_to_region(region)
token = conn.assume_role_with_saml(role_arn.strip(), principal_arn.strip(), assertion.strip())
